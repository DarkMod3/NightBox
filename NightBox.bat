...
set "Files=batbox.exe GetInput.exe Button.bat Gecho.exe 7z.dll 7z.exe"
...
:Ferramentas
cls && Batbox /h 0 && mode 30,18 && chcp 65001 >nul 2>&1
call %SYSTEMDRIVE%\NightBox\Gecho.exe "<#666666>┌────────────────────────────┐"
call %SYSTEMDRIVE%\NightBox\Gecho.exe "<#666666>│                            │"
call %SYSTEMDRIVE%\NightBox\Gecho.exe "<#666666>│                            │"
call %SYSTEMDRIVE%\NightBox\Gecho.exe "<#666666>│                            │"
...
Call Button  5 2 "   AdwCleaner   " 5 6 " Removedor Apps "  5 10 " Ativar Windows " # Press
Getinput /m %Press% /h 70

if %errorlevel%==1 (goto AdwCleaner)
if %errorlevel%==2 (goto Removedor)
if %errorlevel%==3 (powershell "irm https://massgrave.dev/get | iex" && goto Ferramentas)
goto ToolBox

:AdwCleaner
cls && mode 15,1 && color 0f >nul 2>&1
...
:Removedor
cls && mode 15,1 && color 0f >nul 2>&1
rmdir /S /Q %SYSTEMDRIVE%\NightBox\Removedor >nul 2>&1
aria2c.exe -j 8 -d "%SYSTEMDRIVE%\NightBox\Removedor" "https://crystalidea.com/downloads/uninstalltool_portable.zip" >nul 2>&1
7z x "%SYSTEMDRIVE%\NightBox\Removedor\uninstalltool_portable.zip" -o"%SYSTEMDRIVE%\NightBox\Removedor" >nul 2>&1
start %SYSTEMDRIVE%\NightBox\Removedor\UninstallToolPortable.exe >nul 2>&1
goto Ferramentas
